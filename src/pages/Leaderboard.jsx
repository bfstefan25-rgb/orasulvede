...
const rankStyle = (i) => {
  if (i === 0) return { badge: 'bg-yellow-400',                     card: 'bg-yellow-50 border border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-700/50' }
  if (i === 1) return { badge: 'bg-gray-400',                       card: 'bg-gray-50 border border-gray-200 dark:bg-gray-700/40 dark:border-gray-600'            }
  if (i === 2) return { badge: 'bg-orange-400',                     card: 'bg-orange-50 border border-orange-200 dark:bg-orange-900/20 dark:border-orange-700/50' }
  return        { badge: 'bg-gray-200 !text-gray-600 text-gray-600', card: 'bg-white border border-gray-100 dark:bg-gray-800 dark:border-gray-700'                 }
}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
      <div className="max-w-6xl mx-auto px-4 py-8 pb-24 md:pb-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-14 h-14 bg-yellow-100 dark:bg-yellow-900/30 rounded-2xl flex items-center justify-center text-2xl">
            🏆
          </div>
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Clasamente și statistici</h1>
            <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">Topul celor mai activi utilizatori și zone problematice</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Total rapoarte', value: globalStats.total,      color: 'text-blue-600',   bg: 'bg-blue-50 dark:bg-blue-900/20'   },
            { label: 'În lucru',       value: globalStats.inProgress,  color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-900/20' },
            { label: 'Rezolvate',      value: globalStats.resolved,    color: 'text-green-600',  bg: 'bg-green-50 dark:bg-green-900/20'  },
          ].map(({ label, value, color, bg }) => (
            <div key={label} className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="font-bold text-gray-900 dark:text-white text-lg">Top utilizatori</h2>
            </div>
                ...
                {topUsers.map((u, i) => {
                  ...
                  return (
                    <div key={u.id} className={`flex items-center gap-4 p-4 rounded-2xl ${card} ${isMe ? 'ring-2 ring-blue-400' : ''}`}>
                      ...
                      {/* Avatar */}
                      <div className="w-11 h-11 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {u.avatar_url
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="font-bold text-gray-900 dark:text-white text-xl">{(u.points || 0).toLocaleString('ro-RO')}</p>
                        <p className="text-gray-400 dark:text-gray-500 text-xs">puncte</p>
                      </div>
                    </div>
                  )
                })}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="font-bold text-gray-900 dark:text-white text-lg">Cele mai problematice străzi</h2>
            </div>
              ...
              <div className="space-y-5">
                {topStreets.map((street, i) => (
                  <div key={street.name} className="flex items-start gap-3">
                    <span className="w-7 h-7 bg-red-50 dark:bg-red-900/30 text-red-500 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                      {i + 1}
                    </span>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="font-bold text-gray-900 dark:text-white text-lg">Categorii raportate</h2>
            </div>
          ...
          {/* How to earn points — blue gradient stays, no dark override needed */}
