    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
      <div className="max-w-6xl mx-auto px-4 py-6 pb-24 md:pb-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Total probleme', value: reports.length,                                     color: 'text-blue-600',   bg: 'bg-blue-50   dark:bg-blue-900/30' },
            { label: 'În lucru',       value: reports.filter(r => r.status === 'in_lucru').length, color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-900/30' },
            { label: 'Rezolvate',      value: reports.filter(r => r.status === 'rezolvat').length, color: 'text-green-600',  bg: 'bg-green-50  dark:bg-green-900/30' },
        ...
        {/* Feed header */}
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Feed probleme oraș</h1>
        <p className="text-gray-400 dark:text-gray-500 mb-6">Urmărește și votează problemele raportate în comunitatea ta.</p>
        ...
        {/* Reports grid */}
        {loading ? (
          <div className="text-center py-20 text-gray-400 dark:text-gray-500">Se încarcă...</div>
        ) : sorted.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-gray-400 dark:text-gray-500 text-lg mb-2">Nicio problemă raportată încă</p>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {sorted.map(report => (
              <Link
                key={report.id}
                to={`/raport/${report.id}`}
                className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow block"
              >
                {report.image_url && (
                ...
                <div className="p-4">
                  ...
                  <h3 className="font-bold text-gray-900 dark:text-white mt-2 mb-1">{report.title}</h3>
                  {report.address && (
                    <div className="flex items-center gap-1 text-gray-400 dark:text-gray-500 text-sm mb-3">
                      <MapPin size={14} />
                      <span>{report.address}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-gray-400 dark:text-gray-500 text-sm">
